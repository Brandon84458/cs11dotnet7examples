...
foreach (Person i in bob.Children)
{
    WriteLine($">{i.Name}");
}
...
WriteLine($"There are {fruitNamed.Number} of {fruitNamed.Name}.");

var thing1 = ("Neville", 4);
WriteLine($"{thing1.Item1} has {thing1.Item2} children.");
var thing2 = (bob.Name, bob.Children.Count);
WriteLine($"{thing2.Name} has {thing2.Count} chilren.");

(string fruitName, int fruitNumber) = bob.GetFruit();
WriteLine($"Deconstructed: {fruitName}, {fruitNumber}");

// Deconstruction a Person
...
WriteLine($"{lamech.Name} is marreid to {lamech.Spouse?.Name?? "nobody"}");
WriteLine($"{adah.Name} is marreid to {adah.Spouse?.Name?? "nobody"}");
...
int number = 5; // change to -1 to make the execption handling code execute
try
{
...
{
    WriteLine($"{ex.GetType()} says: {ex.Message} number wsa {number}.");
}

Passenger[] passengers = {
    new FirstClassPassenger { AirMiles = 1_419, Name = "Suman" },
    new FirstClassPassenger { AirMiles = 16_562, Name = "Lucky"},
    new BuisnessClassPassenger { Name = "Janice"},
